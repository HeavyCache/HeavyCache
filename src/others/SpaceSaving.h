...
template<int key_len, uint32_t mem_in_bytes>
class SpaceSaving
{
//    int now_element;
    static constexpr int bytes_per_item = 36 + key_len * 2;
    static constexpr int capacity = mem_in_bytes / bytes_per_item;
//    uint16_t tail_idx;
    typedef KeyNode<key_len> SSKeyNode;
    typedef ValueNode<key_len> SSValNode;
    SSKeyNode key_nodes[capacity];
    SSValNode val_nodes[capacity];


//    SSKeyNode * key_node_pool[capacity];
    SSValNode * val_node_pool[capacity];
...
public:
    ...
    SpaceSaving() {
        memset(key_nodes, 0, sizeof(key_nodes));
        memset(val_nodes, 0, sizeof(val_nodes));
//        now_element = 0;
//        nodes[0].val = -1;
